...
const EcontDelivery = () => {
  ...
  // Component state
  const [iframeUrl, setIframeUrl] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)("");
  const [globalShipmentPrices, setGlobalShipmentPrices] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)({
    cod: undefined,
    cod_e: undefined,
    no_cod: undefined
  });
  const [globalInfoMessage, setGlobalInfoMessage] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)("");
  const [isModalOpen, setIsModalOpen] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)(false);
  const [isLoading, setIsLoading] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)(false);
  const [useShipping, setUseShipping] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)(true);
  const [buttonContainer, setButtonContainer] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)(null);
  const [portalContainer, setPortalContainer] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)(null);
  const [isEcontSelected, setIsEcontSelected] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)(false);
  const [globalAlertMessage, setGlobalAlertMessage] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)(false);
  const [cartTotal, setCartTotal] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)(0);
  const [cartWeight, setCartWeight] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)(1); // Default weight of 1kg
  const [checkoutButtonDisabled, setCheckoutButtonDisabled] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)(false);
  const [isOrderButtonDisabled, setIsOrderButtonDisabled] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)(false);
  const [currentShippingMethod, setCurrentShippingMethod] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)("");

  // Refs
  const isMountedRef = (0,react__WEBPACK_IMPORTED_MODULE_0__.useRef)(true);
  const messageListenerRef = (0,react__WEBPACK_IMPORTED_MODULE_0__.useRef)(null);
  const econtContainerRef = (0,react__WEBPACK_IMPORTED_MODULE_0__.useRef)(null);
  ...
  const validateShippingPrice = e => {
    if (isEcontShippingSelected() && (globalShipmentPrices.cod === undefined || globalShipmentPrices.cod_e === undefined || globalShipmentPrices.no_cod === undefined)) {
      e.preventDefault();
      e.stopPropagation();
  ...
  const isEcontShippingSelected = () => {
    ...
    const selectors = ['input[value="delivery_with_econt"]', 'input[id*="delivery_with_econt"]', 'input[name^="shipping_method"][value*="econt"]', 'input[name^="shipping_method"][id*="econt"]'];
  ...
  const togglePlaceOrderButton = disable => {
    ...
    const buttonSelectors = [".wc-block-components-checkout-place-order-button", "#place_order", "button[name='woocommerce_checkout_place_order']", ".checkout-button", "button.alt"];
  ...
  const checkForm = useShipping => {
    const fields = [`#${useShipping ? "shipping" : "billing"}-first_name`, `#${useShipping ? "shipping" : "billing"}-last_name`, `#${useShipping ? "shipping" : "billing"}-country`, `#${useShipping ? "shipping" : "billing"}-address_1`, `#${useShipping ? "shipping" : "billing"}-city`, `#${useShipping ? "shipping" : "billing"}-state`, `#${useShipping ? "shipping" : "billing"}-postcode`, "#billing-phone", "#email"];
    let showModal = true;
    ...
    try {
      ...
      if (typeof jQuery !== "undefined") {
        ...
        if (cartTotalElement.length) {
          const total = Number.parseFloat(cartTotalElement.text().replace(/[^0-9,.]/g, "").replace(",", ".")) || 0;
          setCartTotal(total);
  ...
  const handleIframeMessage = async event => {
    ...
    // Set info message
    const infoMessage = `${data.shipping_price} ${data.shipping_price_currency_sign} за доставка и ${Math.round((shipmentPrice - data.shipping_price) * 100) / 100} ${data.shipping_price_currency_sign} наложен платеж.`;
      ...
      if (placeOrderButton.length) {
        if (disabled) {
          ...
          if (!jQuery("#econt-shipping-warning").length) {
            const warningMessage = locale === "bg" ? "Моля калкулирайте цена за доставка с Еконт преди да продължите!" : "Please calculate Econt shipping price before proceeding!";
            placeOrderButton.after(`<div id="econt-shipping-warning" style="color: #e2401c; margin-top: 10px;">${warningMessage}</div>`);
          }
        } else {
          // Enable the button
  ...
  const checkShippingMethod = () => {
    ...
    if (isEcont) {
      ...
      const shouldDisableButton = globalShipmentPrices.cod === undefined || globalShipmentPrices.cod_e === undefined || globalShipmentPrices.no_cod === undefined;
  ...
  const renderEcontButton = () => {
    ...
    return (0,react_dom__WEBPACK_IMPORTED_MODULE_1__.createPortal)((0,react__WEBPACK_IMPORTED_MODULE_0__.createElement)("button", {
      type: "button",
    ...
    }, globalShipmentPrices.no_cod ? buttonText : selectLocationText), btnContainer);
  };
  ...
  const renderModal = () => {
    ...
    }, locale === "bg" ? "Доставка с Еконт" : "Econt Delivery"), (0,react__WEBPACK_IMPORTED_MODULE_0__.createElement)("button", {
      type: "button",
    ...
    }, locale === "bg" ? "Зареждане..." : "Loading..."), (0,react__WEBPACK_IMPORTED_MODULE_0__.createElement)("div", {
      id: "place_iframe_here",
      style: {
        display: isLoading ? "none" : "block",
        zIndex: isLoading ? "-1" : "1"
      }
    }, iframeUrl && (0,react__WEBPACK_IMPORTED_MODULE_0__.createElement)("iframe", {
      src: iframeUrl,
  ...
  if (portalContainer && isEcontSelected) {
    ...
    }, renderEcontButton(), renderModal()), portalContainer);
  }
